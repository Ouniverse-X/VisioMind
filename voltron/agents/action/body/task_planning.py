"""High-level task-planner backends owned by the Action agent body."""
...
class OpenAICompatibleActionTaskPlanner:
    """Action task planner backed by an OpenAI-compatible endpoint."""
    ...
    def _request_chat_completion(self, user_prompt: str) -> str:
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                raise RuntimeError(f"Action task planner request failed calling {url}: {exc}") from exc
        else:
...
class HeuristicActionTaskPlanner:
    """Minimal non-LLM fallback that preserves the Action planning interface."""
