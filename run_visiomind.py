...
from visiomind.plan import IndustrialInstructionModel
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=(
            ...
            / "action"
            / "configs"
            / "compact_industrial_pliers_to_toolbox_cell3_i00.json"
        ),
    )
    parser.add_argument(
        "--model",
    ...
    environment["VISIOMIND_ACTION_HOME"] = str(ROOT / "visiomind" / "action")
    command = [
        sys.executable,
        "-m",
        "visiomind.action.entrypoints.examples.closed_loop.action_only",
        "--config",
        str(args.config),
        "--action-sequence",
